...
for dirname in sys.argv[1:]:
    ...
    for filename in [fn for fn in os.listdir(dirname) if fn.endswith('.csv')]:
        ...
        results.append(['Flat' if 'flat' in dirname else 'Random', 'a' if 'Experiments' in dirname else 'b' if 'Experiment2' in dirname else 'c', filename, max_pull_force, mean_gripper_current])
...
results_df = pd.DataFrame(data=results, columns=["Scenario", "Material", "Run", "Pulling Force [N]", "Current [A]"])
print(results_df)
seaborn.set_style("whitegrid")
seaborn.axes_style({'font.family': ['serif']})
plt.rcParams['font.family'] = 'serif'
gfg = seaborn.relplot(results_df, x='Current [A]', y='Pulling Force [N]', hue='Material', aspect=1.4, facet_kws={'ylim': (0,4)})
plt.show()
...
# Ignored file: Experiments/random_current_10/logfile_23_04_03_17_26_24.csv